...
import { assessAffordability, buildSchedule, type Frequency, type InterestMethod } from "@/lib/loan-math";
...
function ApplicationsPage() {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, application_number, customer_id, product_id, amount, term_months, repayment_frequency, interest_rate_percent, interest_method, status, affordability_verdict, created_at, customers(full_name, customer_number), loan_products(name)")
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create, review, approve or decline applications.</p>
        </div>
        <Dialog open={openForm} onOpenChange={setOpenForm}>
          <DialogTrigger asChild>
            <Button><Plus className="h-4 w-4 mr-1" /> New application</Button>
          </DialogTrigger>
          <NewApplicationForm onSaved={() => { setOpenForm(false); void load(); }} onClose={() => setOpenForm(false)} />
        </Dialog>
      </div>
        ...
        <CardHeader className="gap-3">
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[240px]">
              ...
              <Input className="pl-8" placeholder="Search by application #, customer, product…" value={q} onChange={(e) => setQ(e.target.value)} />
            </div>
                ...
                <TableBody>
                  {filtered.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-right tabular-nums">{formatNAD(Number(r.amount))}</TableCell>
                      <TableCell className="text-right">{r.term_months} mo</TableCell>
                      <TableCell><Badge variant="outline" className={STATUS_STYLES[r.status] ?? ""}>{r.status.replace(/_/g, " ")}</Badge></TableCell>
                      <TableCell><span className="text-xs capitalize">{r.affordability_verdict ?? "—"}</span></TableCell>
                      <TableCell className="text-xs text-muted-foreground">{formatDate(r.created_at)}</TableCell>
                      <TableCell className="text-right">
                        <Link to="/applications/$applicationId" params={{ applicationId: r.id }}>
                  ...
                  {filtered.length === 0 && (
                    <TableRow><TableCell colSpan={9} className="text-center text-sm text-muted-foreground py-8">No applications match.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </div>
...
type ProductLite = {
  id: string; name: string; interest_rate_percent: number; interest_method: InterestMethod;
  min_amount: number; max_amount: number; min_term_months: number; max_term_months: number; repayment_frequency: Frequency;
};
...
function NewApplicationForm({ onSaved, onClose }: { onSaved: () => void; onClose: () => void }) {
  ...
  const [v, setV] = useState({
    customer_id: "", product_id: "", amount: "5000", term_months: "6", purpose: "",
    monthly_income: "0", monthly_expenses: "0", existing_debt: "0",
  });
  ...
  useEffect(() => {
    void (async () => {
      const [c, p] = await Promise.all([
        supabase.from("customers").select("id, full_name, customer_number, monthly_income").order("full_name").limit(500),
  ...
  const preview = useMemo(() => {
    ...
    const amt = Number(v.amount), term = Number(v.term_months);
  ...
  const submit = async (status: "draft" | "submitted") => {
    ...
    if (parsed.data.amount < product.min_amount || parsed.data.amount > product.max_amount) {
      toast.error(`Amount must be between ${formatNAD(product.min_amount)} and ${formatNAD(product.max_amount)}`); return;
    }
    if (parsed.data.term_months < product.min_term_months || parsed.data.term_months > product.max_term_months) {
      toast.error(`Term must be between ${product.min_term_months} and ${product.max_term_months} months`); return;
    }
    setSaving(true);
    const payload = {
      ...parsed.data,
    ...
    const { data, error } = await supabase.from("loan_applications").insert(payload).select("id, application_number").single();
    setSaving(false);
    if (error) { toast.error(error.message); return; }
    await logAudit("loan.apply", { entity: "loan_application", entity_id: data.id, meta: { number: data.application_number, status } });
    toast.success(`Application ${data.application_number} ${status === "submitted" ? "submitted" : "saved as draft"}.`);
    onSaved();
  };
  ...
  return (
    <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription>Fill in the loan details and affordability inputs. A preview will appear below.</DialogDescription>
      </DialogHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <F label="Customer *">
          <Select value={v.customer_id} onValueChange={onCustomer}>
            ...
            <SelectContent>{customers.map((c) => <SelectItem key={c.id} value={c.id}>{c.full_name} · {c.customer_number}</SelectItem>)}</SelectContent>
          </Select>
        </F>
        <F label="Product *">
        ...
        <F label={`Amount (N$)${product ? ` · ${formatNAD(product.min_amount)}–${formatNAD(product.max_amount)}` : ""}`}>
          <Input type="number" value={v.amount} onChange={(e) => set("amount", e.target.value)} />
        </F>
        <F label={`Term (months)${product ? ` · ${product.min_term_months}–${product.max_term_months}` : ""}`}>
          <Input type="number" value={v.term_months} onChange={(e) => set("term_months", e.target.value)} />
        </F>
        <F label="Purpose" className="md:col-span-2">
          <Textarea rows={2} value={v.purpose} onChange={(e) => set("purpose", e.target.value)} placeholder="e.g. School fees, business inventory" />
        </F>
        <F label="Monthly income (N$)"><Input type="number" value={v.monthly_income} onChange={(e) => set("monthly_income", e.target.value)} /></F>
        <F label="Monthly expenses (N$)"><Input type="number" value={v.monthly_expenses} onChange={(e) => set("monthly_expenses", e.target.value)} /></F>
        <F label="Existing debt (N$/mo)"><Input type="number" value={v.existing_debt} onChange={(e) => set("existing_debt", e.target.value)} /></F>
      </div>

      {preview && (
        <div className="rounded-md border p-3 bg-muted/30 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
          <Kv label={`Instalment (${product?.repayment_frequency})`} value={formatNAD(preview.sched.instalment)} />
          ...
          <Kv label={`DTI (verdict: ${preview.aff.verdict})`} value={`${preview.aff.dtiPercent.toFixed(1)}%`} />
        </div>
      )}
...
function Kv({ label, value }: { label: string; value: string }) {
  return <div><div className="text-xs text-muted-foreground">{label}</div><div className="font-bold tabular-nums">{value}</div></div>;
}

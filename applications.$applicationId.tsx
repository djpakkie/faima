...
import { downloadTemplateFile, mergeAgreementDocx, downloadBlob } from "@/lib/agreement-merge";
...
function ApplicationDetail() {
  ...
  const [disburseMethod, setDisburseMethod] = useState<"eft" | "cash">("eft");
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "*, customers(id, full_name, customer_number, phone, email, id_number, physical_address), loan_products(name, processing_fee_percent, insurance_fee_percent, late_fee_percent)",
      )
      .eq("id", applicationId)
      .single();
  ...
  const submit = () =>
    updateStatus({ status: "submitted" }, "loan.apply", { transition: "draft→submitted" });
  const recommend = () =>
    updateStatus(
      {
        status: "recommended",
  ...
  const disburse = async () => {
    ...
    const processingFee =
      Number(app.amount) * (Number(app.loan_products?.processing_fee_percent ?? 0) / 100);
    const insuranceFee =
      Number(app.amount) * (Number(app.loan_products?.insurance_fee_percent ?? 0) / 100);
    ...
    if (loanErr || !loan) {
      setBusy(false);
      toast.error(loanErr?.message ?? "Disbursement failed");
      return;
    }
    ...
    await logAudit("loan.disburse", {
      entity: "loan",
      entity_id: loan.id,
      meta: { loan_number: loan.loan_number, application_id: app.id },
    });
    setBusy(false);
  ...
  const money = (n: number) =>
    new Intl.NumberFormat("en-NA", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(
      n,
    );
  const pct = (n: number) =>
    new Intl.NumberFormat("en-NA", { minimumFractionDigits: 2, maximumFractionDigits: 3 }).format(
      n,
    );
  ...
  const generateAgreement = async () => {
    ...
    try {
      const { data: tmpl, error: tmplErr } = await supabase
        ...
        .select("file_path")
        .eq("key", "loan_agreement")
        .maybeSingle();
      if (tmplErr) throw new Error(tmplErr.message);
      if (!tmpl) {
        toast.error(
          "No loan agreement template uploaded yet. Ask an administrator to upload one under Admin → Document Templates.",
        );
        return;
      }

      const buffer = await downloadTemplateFile(tmpl.file_path);
      ...
      const frequencyLabel =
        app.repayment_frequency === "monthly"
          ? "Monthly"
          : app.repayment_frequency === "biweekly"
            ? "Fortnightly"
            : "Weekly";

      const blob = mergeAgreementDocx(buffer, {
        customer_full_name: app.customers?.full_name ?? "",
        customer_id_number: app.customers?.id_number ?? "",
        customer_address: app.customers?.physical_address ?? "",
        reference_number: app.application_number,
        product_name: app.loan_products?.name ?? "",
        principal_amount: money(Number(app.amount)),
        interest_rate: pct(Number(app.interest_rate_percent)),
        interest_method_label:
          app.interest_method === "reducing_balance" ? "reducing balance" : "flat",
        term_months: String(app.term_months),
        frequency_label: frequencyLabel,
        num_instalments: String(schedule.rows.length),
        instalment_amount: money(schedule.instalment),
        processing_fee_percent: pct(Number(app.loan_products?.processing_fee_percent ?? 0)),
        processing_fee_amount: money(processingFeeAmount),
        insurance_fee_percent: pct(Number(app.loan_products?.insurance_fee_percent ?? 0)),
        insurance_fee_amount: money(insuranceFeeAmount),
        late_fee_percent: pct(Number(app.loan_products?.late_fee_percent ?? 0)),
        total_interest_amount: money(schedule.totalInterest),
        total_repayable_amount: money(schedule.totalRepayable),
        disbursement_date: formatDate(disburseDate),
        first_repayment_date: formatDate(schedule.rows[0].dueDate),
        maturity_date: formatDate(schedule.maturityDate),
        disbursement_method: disburseMethod === "eft" ? "EFT to Borrower's bank account" : "Cash",
      });

      downloadBlob(blob, `${app.application_number}-Loan-Agreement.docx`);
      await logAudit("document.generate", {
        entity: "loan_application",
        entity_id: app.id,
        meta: { template: "loan_agreement" },
      });
      toast.success("Loan agreement generated.");
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Failed to generate the agreement.");
    } finally {
  ...
  const canGenerateAgreement = ["approved", "disbursed"].includes(app.status);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-mono">{app.customers?.customer_number}</span> · Created{" "}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {canRecommend && (
            <Button variant="secondary" onClick={recommend} disabled={busy}>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Provide a reason — this will be recorded and visible to the customer file.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <Textarea
                  rows={3}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <Kv
              label="Rate"
              value={`${Number(app.interest_rate_percent).toFixed(2)}% p.a. (${app.interest_method === "reducing_balance" ? "reducing" : "flat"})`}
            />
            ...
            {app.purpose && (
              <div>
                <div className="text-xs text-muted-foreground pt-2">Purpose</div>
                <div>{app.purpose}</div>
              </div>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <Kv
              label="DTI"
              value={
                app.affordability_ratio != null
                  ? `${Number(app.affordability_ratio).toFixed(1)}%`
                  : "—"
              }
            />
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Two-step approval when in review.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            <Kv
              label="Recommended"
              value={app.recommended_at ? formatDate(app.recommended_at) : "—"}
            />
            <Kv label="Approved" value={app.approved_at ? formatDate(app.approved_at) : "—"} />
            {app.declined_reason && (
              <div>
                <div className="text-xs text-muted-foreground pt-2">Decline reason</div>
                <div className="text-red-600">{app.declined_reason}</div>
              </div>
            )}

            {canGenerateAgreement && (
              <div className="pt-3 border-t space-y-2">
                <Label className="text-xs">Disbursement method (shown on agreement)</Label>
                <Select
                  value={disburseMethod}
                  onValueChange={(v) => setDisburseMethod(v as "eft" | "cash")}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="eft">EFT to Borrower's bank account</SelectItem>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Fills the uploaded template with this application's details as a .docx, ready to
                  print for the customer's signature.
                </p>
              </div>
            )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {schedule.numPeriods} instalments · matures {formatDate(schedule.maturityDate)}
          </CardDescription>
        </CardHeader>
